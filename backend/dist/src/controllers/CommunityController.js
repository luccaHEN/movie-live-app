...
class CommunityController {
    ...
    async getStreamerMovies(req, res) {
        const { streamerId } = req.params;
        try {
            const id = parseInt(streamerId, 10);
            const streamer = await prisma_1.prisma.user.findUnique({
                where: { id },
    ...
    async getStreamerMoviesByName(req, res) {
        const { streamerName } = req.params;
        try {
            const streamer = await prisma_1.prisma.user.findFirst({
