...
export class GameController {
  public async guess(req: Request, res: Response): Promise<Response | any> {
    ...
    try {
      ...
      const guessedMovie = await prisma.movie.findFirst({
        where: { id: parseInt(movieId as string, 10), userId }
      });
      ...
      // Comparar Gêneros
      let genreStatus: 'match' | 'partial' | 'wrong' = 'wrong';
      ...
      // Comparar Duração
      let runtimeStatus: 'match' | 'higher' | 'lower' = 'match';
      ...
      let ratingStatus: 'match' | 'higher' | 'lower' = 'match';
      ...
      const requestedByStatus: 'match' | 'wrong' = 
        (guessedMovie.requestedBy?.trim().toLowerCase() === dailyMovie.requestedBy?.trim().toLowerCase()) ? 'match' : 'wrong';
  ...
  public async getDailyPoster(req: Request, res: Response): Promise<Response | any> {
    const userId = (req as any).userId;
    try {
      const watchedMovies = await prisma.movie.findMany({
  ...
  public async getDailySynopsis(req: Request, res: Response): Promise<Response | any> {
    const userId = (req as any).userId;
    try {
      const watchedMovies = await prisma.movie.findMany({
      ...
      while (attempts < 10) {
        ...
        try {
          ...
          if (tmdbRes.data.overview && tmdbRes.data.overview.length > 20) {
            ...
            synopsis = originalText.replace(/\b[A-ZÁÉÍÓÚÂÊÎÔÛÃÕÇ][a-záéíóúâêîôûãõç]*\b/g, (match: string) => {
                if (ignoreList.includes(match)) return match;
                return '[██████]';
            });
            break;
          }
  ...
  public async guessSynopsis(req: Request, res: Response): Promise<Response | any> {
    const userId = (req as any).userId;
    const { movieId } = req.body;
